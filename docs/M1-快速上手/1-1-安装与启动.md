...
![截图：终端输出 node 版本号，如 v20.x.x](screenshots/1-1-node-version.png)
...
![截图：终端输出 Claude Code 版本号](screenshots/1-1-claude-version.png)
...
![截图：GitHub Releases 页面，红框标注 .msi 下载链接](screenshots/1-1-ccswitch-download.png)
...
![截图：CC Switch 主界面，红框标注 + 按钮](screenshots/1-1-ccswitch-add-1.png)

4. 在「Base URL」中填写公司 new-api 平台的地址（问 IT 获取，格式类似 `http://192.168.21.155:3000`）
5. 在「API Key」输入框中粘贴上一步复制的 Key

![截图：添加供应商对话框，红框标注 Base URL 和 API Key 输入框](screenshots/1-1-ccswitch-add-2.png)
...
![截图：供应商卡片启用状态，红框标注启用按钮和高亮效果](screenshots/1-1-ccswitch-enable.png)
...
![截图：CC Switch 设置页面，红框标注"跳过初次安装确认"开关](screenshots/1-1-ccswitch-skip-intro.png)
...
![截图：终端中 Claude Code 成功回复的第一条消息](screenshots/1-1-first-message.png)
